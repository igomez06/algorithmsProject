...
public class MainProgram {
	...
	public double maxAverageGrade(int H, int N) {
		...
		if (N==1) {
			Cell cell = new Cell();
			cell.maxGrade = f(H, projects.get(0), N);
			mag[H][N] = cell;
		...
		for (int i = 0; i <=H; ++i) {
			...
			tempCell.maxGrade = maxAverageGrade(H-i, N-1) + f(i,projects.get(N-1), N);
	...
	public double f(int h, Project n, int place) {
		double grade;
		//grade = (h/((double)H))*n.weight;
		grade = (((double)h)*n.weight)/place;
		return grade;
	}
	
	public static void main(String[] args) {
		MainProgram mp = new MainProgram();
		Project algorithms = new Project(.05);
